...
client = genai.Client(api_key=os.environ['NEW3_AP_KEY'])
...
def main(session, details):
        ...
        while True:
            ...
            if ("let's start the exercise" in response.text.lower() or "next picture" in response.text.lower()):
                ...
                if img_name is None:
                    goodbye = chat.send_message("There are no more images, tell the patient you have practiced enough today.")
                    yield text_to_speech(session, details, goodbye.text)
                    break
                else:
